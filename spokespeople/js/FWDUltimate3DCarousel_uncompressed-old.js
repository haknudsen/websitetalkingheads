...
}(window));/* FWDU3DCarComboBoxButton */
(function (){
...
}(window));/* FWDU3DCarComboBoxSelector */
(function (){
	...
	var FWDU3DCarDisplayObject = function(type, position, overflow, display){
		...
		this.addChildAtZero = function(e){
			if(this.numChildren == 0){
				...
				if(this.contains(e)){this.children_ar.splice(FWDU3DCarUtils.indexOfArray(this.children_ar, e), 1);}	
...
(function (window){
	...
	var FWDU3DCarInfoWindow = function(
			margins,
			backgroundColor_str,
			backgroundOpacity,
			borderRadius,
		...
		var _numExp = /(?:\d|\-\d|\.\d|\-\.\d)+/g,
			...
			_convertToPixels = function(t, p, v, sfx, recurse) {
				...
				if (sfx === "%" && p.indexOf("border") !== -1) {
					...
					style.cssText = "border-style:solid; border-width:0; position:absolute; line-height:0;border-radius:5px";
...
}(window));/* FWDU3DCarSlideshowButton */
(function(window)
{
...
(function(window)
{
	var FWDU3DCarThumbsManager = function(data, parent)
	{
		...
		this.showControls = function()
		{
			...
			{
				FWDU3DCarModTweenMax.to(self.controlsDO, .8, {y:self.stageHeight/2 - self.controlsHeight, ease : Expo.easeOut});
			}
		};
		...
		this.positionControls = function()
		{
			...
			if (!self.data.controlsPos)
			{
				self.controlsDO.setY(self.stageHeight/2 - self.controlsHeight);
			}
		};
		...
		this.update = function(e)
		{
			...
			if (self.data.controlsPos)
			{
				newY = Math.floor((self.stageHeight/2 - self.controlsHeight)/2 + self.controlsHeight + self.carYOffset);
			}
			else
			{
				newY = Math.floor((self.stageHeight/2 - self.controlsHeight)/2) + self.carYOffset;
			}
	...
	var FWDUltimate3DCarousel = function(props)
	{
		...
		this.resizeHandler = function()
		{
			...
			if (self.preloaderLoaded)
			{
				...
				if (self.propsObj.thumbnailsBackgroundImagePath)
				{
					...
					if (self.data.controlsPos)
					{
						self.thumbsBgDO.setY(Math.floor((self.stageHeight/2 - self.originalHeight)/2 + self.data.controlsHeight));
					}
					else
					{
						self.thumbsBgDO.setY(Math.floor((self.stageHeight/2 - self.originalHeight)/2));
					}
				}
		...
		this.positionPreloader = function()
		{
			if (self.preloaderDO)
			{
				...
				if (self.data.controlsPos)
				{
					self.preloaderDO.setY(parseInt((self.stageHeight/2 - self.preloaderDO.getHeight() - self.data.controlsHeight) / 2 + self.data.controlsHeight) + 7);
				}
				else
				{
					self.preloaderDO.setY(parseInt((self.stageHeight/2 - self.preloaderDO.getHeight() - self.data.controlsHeight) / 2) + 7);
				}
			}
		};
